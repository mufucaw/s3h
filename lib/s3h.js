 ...
 * @param {object} globalUploadOptions
 */
async function uploadUntilFinished(
  ...
  if ('maximumConcurrentUploads' in globalUploadOptions) {
    maximumConcurrentUploads = globalUploadOptions.maximumConcurrentUploads;
    delete globalUploadOptions.maximumConcurrentUploads;
  }
  ...
  return new Promise(async (resolve, reject) => {
    while (filePaths.length > 0 || uploadsInProgress > 0) {
      if (uploadsInProgress < maximumConcurrentUploads &&
        ...
        const uploadOptionsForThisFile = { ...globalUploadOptions };
        const s3Key = getS3Key(remoteDirectory, filePath);
        uploadOptionsForThisFile.Bucket = s3Bucket;
        uploadOptionsForThisFile.Key = s3Key;
        uploadOptionsForThisFile.Body = fs.readFileSync(filePath);

        if (!('ContentType' in globalUploadOptions)) {
          uploadOptionsForThisFile.ContentType = mimeTypes.lookup(filePath);
        }

        s3Client.putObject(uploadOptionsForThisFile, (error) => {
          if (error) {
