...
async function uploadDirectory(
  ...
  const allFilePaths = recursiveReadDirSync(process.cwd());
...
async function uploadUntilFinished(
    ...
    remoteDirectory,
    options = {
      maximumConcurrentUploads: DEFAULT_MAXIMUM_CONCURRENT_UPLOADS,
    },
) {
  const failures = {};
  ...
  return new Promise(async (resolve, reject) => {
    while (filePaths.length > 0 || uploadsInProgress > 0) {
      if (uploadsInProgress < options.maximumConcurrentUploads &&
        filePaths.length > 0) {
        uploadsInProgress++;
